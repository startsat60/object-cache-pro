...
class DigestWatcher extends Notify
{
     ...
     * @return object
     */
    protected function getRelayMemoryLimit(Measurements $measurements)
    {
        $memoryLimit = $this->usingRelay
            ? $measurements->latest('relay->memoryLimit')
            : null;

        return (object) [
            'Metric' => WP_CLI::colorize('%pRelay%n: Memory limit'),
            'Median' => is_null($memoryLimit) ? '' : size_format($memoryLimit),
        ];
    }
    ...
    protected function getRelayMemoryHuman(Measurements $measurements)
    {
        ...
        if ($this->usingRelay) {
            ...
            $memoryLimit = $measurements->latest('relay->memoryLimit');

            if ($memoryActive && $memoryLimit) {
                $memoryHuman = sprintf(
                    '%s / %s',
                    size_format($memoryActive),
                    size_format($memoryLimit)
                );
            }
        }
